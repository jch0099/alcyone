				...
				if (_this.YQ.requestType === "img") {
					...
					if (_this.YQ.requestType === "ajax") {
						...
						ifr.load(function(){
							...
							//根据内容设置iframe高宽
							if(_this.YQ.autoFix){
								parent.$("iframe[name="+_this.YQ.ifameName+"]").removeClass(_this.YQ.iframeclassName);
								parent.$("iframe[name="+_this.YQ.ifameName+"]").css("height",0+"px");
								parent.$("iframe[name="+_this.YQ.ifameName+"]").css("width",0+"px");
								parent.$("iframe[name="+_this.YQ.ifameName+"]").css("height",$it.outerHeight(true)+"px");
								parent.$("iframe[name="+_this.YQ.ifameName+"]").css("width",$it.outerWidth(true)+"px");
								if(parent.$("iframe[name="+_this.YQ.ifameName+"]").height()+50>$(window).height()){
									parent.$("iframe[name="+_this.YQ.ifameName+"]").css("height",$(window).height()-50+'px');
								}
								parent.$('#content').css('height',parent.$("iframe[name="+_this.YQ.ifameName+"]").height()+'px');
								//初始化居中
								var left = (parent.$(window).width()-parent.$("#_showBox").width())/2;
								var top = (parent.$(window).height()-parent.$("#_showBox").height())/2;
								parent.$("#_showBox").css({
									"left": left,
									"top": top
								});
