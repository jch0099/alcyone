       ...
       <form id="searchForm" role="_searchForm" action="list" method="post">
         ...
         <div class="containerWrap center-block">
           ...
           <div class="cm-f14 cm-pdt10 cm-tar cm-overflow">
           	 <a href="edit" title="新增图片" class="pull-right cm-btn-confirm-bg cm-btn-default" target="_box">新增图片</a>
           </div>
           <!-- 列表 -->
           <table class="table table-striped table-detailList">
             ...
             <c:forEach var="result" items="${page.results }">
	            <tr>
	              <td>${result.id }</td>
	              <td>${result.title }</td>
	              <td><c:if test="${!empty result.img }"> <img alt="" src="${path }/uploads/${result.img }"> </c:if></td>
	              <td>${path }/uploads/${result.img }</td>
	              <td>
	              ...
	              <td class="detailList-btn">
	                <a href="edit?id=${result.id }" title="編輯图片" class="pull-left icon-edit icon" target="_box"></a>
