...
public class FileUtil {
	 ...
	 * @param bs
	 */
	public static String newFile(String path,String ext,byte[] bs){
		String strFileName = UUID.randomUUID().toString() + "."+ext;
		File file = new File(path);
		file.mkdirs();
		try {
			FileOutputStream fouts = new FileOutputStream(path+strFileName);
			fouts.write(bs);
			fouts.flush();
			fouts.close();
		} 
		catch (IOException e) {
			_Log.error("save file error: ", e);
		}
		return strFileName;
	}
	// 检测文件是否存在
    ...
    public static void dealUploadFile(String filepath,String filename,String oldfilename){
    	if(null != filename && filename.length() > 0){
    		if(!filename.equals(oldfilename)){
    			if(null != oldfilename && oldfilename.length() > 0){
    				//刪除旧图
    				FileUtil.delFile(filepath, oldfilename);
    			}//临时文件移入
    			FileUtil.moveFile(Const.UPLOAD_FLODER_ROOT + filename, filepath + filename);
        	}
    	}else{
    		if(null != oldfilename && oldfilename.length() > 0){
				//刪除旧图
