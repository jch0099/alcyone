...
public class UploadFile {
	...
	public void saveFile() throws Exception {
		...
		for (Map.Entry<String, MultipartFile> entity : this.fileMap.entrySet()) {
			...
			// 扩展名格式：
			extName = this.oriName.substring(this.oriName.lastIndexOf(".")).toLowerCase();
			boolean flag = false;
			for (Object key : UploadFile.extMap.keySet()) {
				flag = flag||Arrays.<String> asList(UploadFile.extMap.get(key).split(",")).contains(extName);
			}
			if(!flag){
				throw new Exception("不允許上載"+extName+"檔案");
			}
			if (this.oriName == null || this.oriName.trim().equals("")) {
				continue;
			}
